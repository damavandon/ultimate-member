...
if ( ! class_exists( 'KIANFR_Shortcoder' ) ) {
  class KIANFR_Shortcoder extends KIANFR_Abstract{
    ...
    public function __construct( $key, $params = array() ) {
      ...
      add_action( 'wp_ajax_kianfr-get-shortcode-'. $this->unique, array( $this, 'get_shortcode' ) );

      if ( ! empty( $this->args['show_in_editor'] ) ) {

        KIANFR::$shortcode_instances[$this->unique] = wp_parse_args( array( 'hash' => md5( $key ), 'modal_id' => $this->unique ), $this->args );

        // elementor editor support
        if ( KIANFR::is_active_plugin( 'elementor/elementor.php' ) ) {
          add_action( 'elementor/editor/before_enqueue_scripts', array( 'KIANFR', 'add_admin_enqueue_scripts' ) );
          add_action( 'elementor/editor/footer', array( 'KIANFR_Field_icon', 'add_footer_modal_icon' ) );
    ...
    ?>
      <div id="kianfr-modal-<?php echo esc_attr( $this->unique ); ?>" class="wp-core-ui kianfr-modal kianfr-shortcode hidden<?php echo esc_attr( $single_usage . $class ); ?>" data-modal-id="<?php echo esc_attr( $this->unique ); ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( 'kianfr_shortcode_nonce' ) ); ?>">
          ...
          <div class="kianfr-modal-table-cell">
            ...
            <div class="kianfr-modal-inner">
              ...
              <div class="kianfr-modal-content">
                <div class="kianfr-modal-loading"><div class="kianfr-loading"></div></div>
                <div class="kianfr-modal-load"></div>
              </div>
              <div class="kianfr-modal-insert-wrapper hidden"><a href="#" class="button button-primary kianfr-modal-insert"><?php echo $this->args['insert_title']; ?></a></div>
            </div>
          </div>
        </div>
      </div>
    ...
    public static function once_editor_setup() {
      ...
      if ( function_exists( 'register_block_type' ) ) {
        add_action( 'enqueue_block_editor_assets', array( 'KIANFR_Shortcoder', 'add_guteberg_blocks' ) );
      }

      if ( kianfr_wp_editor_api() ) {
        add_action( 'media_buttons', array( 'KIANFR_Shortcoder', 'add_media_buttons' ) );
      }
    ...
    public static function add_guteberg_blocks() {
      ...
      foreach ( KIANFR::$shortcode_instances as $value ) {
        ...
        register_block_type( 'kianfr-gutenberg-block/block-'. $value['hash'], array(
