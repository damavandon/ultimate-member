...
if ( ! class_exists( 'KIANFR' ) ) {
  class KIANFR {
    ...
    public function __construct() {
      ...
      add_action( 'after_setup_theme', array( 'KIANFR', 'setup' ) );
      add_action( 'init', array( 'KIANFR', 'setup' ) );
      add_action( 'switch_theme', array( 'KIANFR', 'setup' ) );
      add_action( 'admin_enqueue_scripts', array( 'KIANFR', 'add_admin_enqueue_scripts' ) );
      add_action( 'wp_enqueue_scripts', array( 'KIANFR', 'add_typography_enqueue_styles' ), 80 );
      add_action( 'wp_head', array( 'KIANFR', 'add_custom_css' ), 80 );
      add_filter( 'admin_body_class', array( 'KIANFR', 'add_admin_body_class' ) );
    ...
    public static function includes() {
      ...
      $fields = apply_filters( 'kianfr_fields', array(
        'accordion',
        'background',
        'backup',
        'border',
    ...
    public static function add_admin_enqueue_scripts() {
      ...
      if ( apply_filters( 'kianfr_fa4', false ) ) {
        wp_enqueue_style( 'kianfr-fa', 'https://cdn.jsdelivr.net/npm/font-awesome@4.7.0/css/font-awesome'. $min .'.css', array(), '4.7.0', 'all' );
      } else {
        wp_enqueue_style( 'kianfr-fa5', 'https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@5.15.4/css/all'. $min .'.css', array(), '5.15.5', 'all' );
        wp_enqueue_style( 'kianfr-fa5-v4-shims', 'https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@5.15.4/css/v4-shims'. $min .'.css', array(), '5.15.5', 'all' );
      }

      // Main style
      wp_enqueue_style( 'kianfr', self::include_plugin_url( 'assets/css/style'. $min .'.css' ), array(), self::$version, 'all' );

      // Main RTL styles
      if ( is_rtl() ) {
        wp_enqueue_style( 'kianfr-rtl', self::include_plugin_url( 'assets/css/style-rtl'. $min .'.css' ), array(), self::$version, 'all' );
      }

      // Main scripts
      wp_enqueue_script( 'kianfr-plugins', self::include_plugin_url( 'assets/js/plugins'. $min .'.js' ), array(), self::$version, true );
      wp_enqueue_script( 'kianfr', self::include_plugin_url( 'assets/js/main'. $min .'.js' ), array( 'kianfr-plugins' ), self::$version, true );

      // Main variables
      wp_localize_script( 'kianfr', 'kianfr_vars', array(
        'color_palette'     => apply_filters( 'kianfr_color_palette', array() ),
        'i18n'              => array(
    ...
    public static function add_typography_enqueue_styles() {
      ...
      if ( ! empty( self::$webfonts ) ) {
        ...
        if ( ! empty( self::$webfonts['enqueue'] ) ) {
          ...
          wp_enqueue_style( 'kianfr-google-web-fonts', esc_url( add_query_arg( $query, '//fonts.googleapis.com/css' ) ), array(), null );
        ...
        if ( ! empty( self::$webfonts['async'] ) ) {
          ...
          wp_enqueue_script( 'kianfr-google-web-fonts', esc_url( '//ajax.googleapis.com/ajax/libs/webfont/1.6.26/webfont.js' ), array(), null );

          wp_localize_script( 'kianfr-google-web-fonts', 'WebFontConfig', array( 'google' => array( 'families' => $fonts ) ) );
